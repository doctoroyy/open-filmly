...
import { EnhancedMediaCard } from "@/components/enhanced-media-card"
import { EnhancedLoadingGrid, EmptyStateGrid, SearchLoadingGrid } from "@/components/enhanced-loading-grid"
    ...
    <TooltipProvider>
      <main className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="flex h-screen">
          {/* 增强的侧边栏 */}
          <motion.aside 
            className="w-64 h-full bg-sidebar-background/95 backdrop-blur-sm border-r border-sidebar-border/50 p-4"
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>浏览文件</TooltipContent>
                  </Tooltip>
              ...
              <AnimatePresence>
                {searchQuery && (
                  ...
                  >
                    ...
                    {searchLoading ? (
                      <SearchLoadingGrid query={searchQuery} />
                    ) : searchResults.length > 0 ? (
                      <div className={cn(
                        "grid gap-6",
                        viewMode === 'grid' && "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7",
                        ...
                        {searchResults.map((media) => (
                          <EnhancedMediaCard 
                            key={media.id} 
                            media={media} 
                            variant={viewMode === 'list' ? 'compact' : viewMode}
                      ...
                      <EmptyStateGrid 
                        title="未找到相关内容"
                        description={`没有找到与 "${searchQuery}" 相关的媒体`}
                      />
                    )}
                  </motion.section>
                )}
                ...
                <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-8">
                      ...
                      <>
                        ...
                        <motion.section variants={sectionVariants}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <motion.div 
                              className="relative h-48 rounded-xl overflow-hidden cursor-pointer group bg-gradient-to-br from-red-500 to-orange-600"
                            ...
                            <motion.div 
                              className="relative h-48 rounded-xl overflow-hidden cursor-pointer group bg-gradient-to-br from-blue-500 to-purple-600"
                              whileHover={{ scale: 1.02 }}
                            ...
                            <motion.div 
                              className="relative h-48 rounded-xl overflow-hidden cursor-pointer group bg-gradient-to-br from-green-500 to-yellow-500"
                              whileHover={{ scale: 1.02 }}
                  ...
                  <TabsContent value="movies">
                    {loading ? (
                      ...
                      <EmptyStateGrid 
                        title="暂无电影"
                        description="配置SMB连接后将自动发现和添加电影"
                        action={
                  ...
                  <TabsContent value="tv">
                    {loading ? (
                      ...
                      <EmptyStateGrid 
                        title="暂无电视剧"
                        description="配置SMB连接后将自动发现和添加电视剧"
                        action={
                          <Button onClick={() => setShowFileBrowser(true)}>
                            <FolderOpen className="w-4 h-4 mr-2" />
                            浏览文件
                  ...
                  <TabsContent value="recent">
                    {loading ? (
                      ...
                      <EmptyStateGrid 
                        title="暂无最近观看"
                        description="开始观看媒体后，这里将显示您的观看历史"
                      />
                    )}
                  </TabsContent>
                </Tabs>
        ...
        <AnimatePresence>
          {showFileBrowser && (
            ...
            >
              <motion.div 
                className="bg-background rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl"
