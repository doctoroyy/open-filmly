...
import { Folder, File, ArrowLeft, FolderOpen, CheckSquare, Square, RefreshCw, AlertCircle, Grid, List, Home, ChevronRight, Settings } from "lucide-react"
...
interface NetworkFileBrowserProps {
  ...
  storageType?: 'smb' | 'ftp' | 'nfs' | 'webdav'
  title?: string
}

export function NetworkFileBrowser({
  initialPath = "/",
  selectionMode = false,
  onSelect,
  onCancel,
  ...
  const [currentStorageType, setCurrentStorageType] = useState<string>(storageType)
  ...
  // Load directory contents using unified network storage API
  const loadDirectoryContents = async (path: string, retryCount: number = 0) => {
    ...
    try {
      console.log(`Loading directory: path="${path}", storageType="${currentStorageType}"`)
      
      // Use the unified network storage API
      const result = await window.electronAPI?.getNetworkDirContents({
        path,
      ...
      if (result?.success && result.items) {
        ...
        if (errorMessage.includes("STATUS_OBJECT_NAME_NOT_FOUND") || errorMessage.includes("not found")) {
          ...
          setError(`Access denied. Please check your ${currentStorageType.toUpperCase()} credentials and permissions.`)
        } else if (errorMessage.includes("network") || errorMessage.includes("connection")) {
          setError(`Network connection error. Please check your ${currentStorageType.toUpperCase()} server connection.`)
        } else if (errorMessage.includes("binary not found") || errorMessage.includes("provider not available")) {
          setError(`${currentStorageType.toUpperCase()} provider not available. Please check your installation.`)
        } else {
          setError(`${currentStorageType.toUpperCase()}: ${errorMessage}`)
        }
  ...
  // Load when storage type changes
  useEffect(() => {
  ...
  const handleStorageTypeChange = (newType: string) => {
    ...
    // Reset to root path when switching storage types
    setCurrentPath("/")
    setPathHistory([])
  }
  ...
  const handleAddFile = async (file: FileItem) => {
    try {
      ...
      setLoading(true)
      const result = await window.electronAPI?.addSingleNetworkMedia({
        filePath,
        storageType: currentStorageType
      })
  ...
  const getStorageTypeDisplayName = (type: string) => {
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-xs">
              {getStorageTypeDisplayName(currentStorageType)}
            </Badge>
          </CardTitle>
          <div className="flex items-center gap-2">
            {/* Storage Type Selector */}
            <Select value={currentStorageType} onValueChange={handleStorageTypeChange}>
